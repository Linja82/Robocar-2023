...
void lineTrack(){
  ...
  if (leftCurrent > (avgLeftLine + lightVariance) || leftCurrent < (avgLeftLine - lightVariance)) {   // Left tracker detected black tape
    drive("DIFFERENTIAL LEFT", 35, 0);    // Drive straight with a slight left turn
  }
  else if (rightCurrent > (avgRightLine + lightVariance) || rightCurrent < (avgRightLine - lightVariance)) {    // Right tracker detected black tape
